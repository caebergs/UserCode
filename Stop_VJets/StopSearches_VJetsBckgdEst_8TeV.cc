...
int StopSearches_VJetsBckgdEst_NoFiles_SevJets()
{
  ...
  const UInt_t NbOfPE = 100;
  const UInt_t NbOfJetBins = 3;
  ...
  std::vector<std::vector<TFile*> > files(5, std::vector<TFile*>());
  std::vector<std::vector<std::string> > samples_names(5, std::vector<std::string>());
  std::vector<std::vector<std::string> > samples_labelBQ(5, std::vector<std::string>());
  std::vector<std::vector<Double_t> > samples_weights(5, std::vector<Double_t>());
  ...
  Double_t Wn_SF = 1.34 ; // To be checked on 8 TeV data !!!!!!!!
  
  Double_t entries = -1.;
  
  TFile *fBtagEff = new TFile((pathCMSSM+"Leg3_8TeV_NoMods/test/BTagEff_MCsamples.root").c_str(), "read");
  ...
  if (WnJets) {
    ...
    samples_weights[1].push_back((entries==0.? 0. : Wn_SF*1435. /entries )); //W2jets
    ...
    entries = ((TH1D*) (* files[1].rbegin())->Get("Entries"))->GetBinContent(2);
    samples_weights[1].push_back((entries==0.? 0. : Wn_SF*304.2 /entries )); //W3jets
    ...
    entries = ((TH1D*) (* files[1].rbegin())->Get("Entries"))->GetBinContent(2);
    samples_weights[1].push_back((entries==0.? 0. : Wn_SF*172.6 /entries )); //W4jets
  } else {
  ...
  if (WnJets) {
    ...
    samples_weights[2].push_back((entries==0.? 0. : Wn_SF*1435. /entries )); //W2jets
    ...
    entries = ((TH1D*) (* files[2].rbegin())->Get("Entries"))->GetBinContent(2);
    samples_weights[2].push_back((entries==0.? 0. : Wn_SF*304.2 /entries )); //W3jets
    ...
    entries = ((TH1D*) (* files[2].rbegin())->Get("Entries"))->GetBinContent(2);
    samples_weights[2].push_back((entries==0.? 0. : Wn_SF*172.6 /entries )); //W4jets
  } else {
  ...
  /**
   ...
   datafiles.push_back(TFile::Open((leg3Dir+"MuHad_13Jul_RunA.root").c_str(), "READ"));
   weight_data.push_back( 1 );
   
   dataNames.push_back("MuHad_06Aug_RunA");
   datafiles.push_back(TFile::Open((leg3Dir+"MuHad_06Aug_RunA.root").c_str(), "READ"));
   weight_data.push_back( 1 );
   }
   
   if (runPeriod.Contains("B")) {
   dataNames.push_back("Single Mu RunB");
   datafiles.push_back(TFile::Open((leg3Dir+"SingleMu_13Jul_RunB.root").c_str(), "READ"));
   weight_data.push_back( 1 );
   }
   ...
   datafiles.push_back(TFile::Open((leg3Dir+"SingleMu_PRv2_RunC.root").c_str(), "READ"));
   weight_data.push_back( 1 );
   }
   
   if (runPeriod.Contains("D")) {
   dataNames.push_back("Single Mu RunD");
   datafiles.push_back(TFile::Open((leg3Dir+"SingleMu_RunD.root").c_str(), "READ"));
   weight_data.push_back( 1 ); //
   }
   //      }
   //      if (((channel+1)&2)!=0) {
   ...
   datafiles.push_back(TFile::Open((leg3Dir+"EleHad_13Jul_RunA.root").c_str(), "READ"));
   weight_data.push_back( 1 );
   
   dataNames.push_back("ElHad_06Aug_RunA");
   datafiles.push_back(TFile::Open((leg3Dir+"EleHad_06Aug_RunA.root").c_str(), "READ"));
   weight_data.push_back( 1 );
   }
   
   if (runPeriod.Contains("B")) {
   dataNames.push_back("Single El RunB");
   datafiles.push_back(TFile::Open((leg3Dir+"SingleEl_13Jul_RunB.root").c_str(), "READ"));
   weight_data.push_back( 1 );
   }
   ...
   datafiles.push_back(TFile::Open((leg3Dir+"SingleEl_PRv2_RunC.root").c_str(), "READ"));
   weight_data.push_back( 1 );
   }
   
   if (runPeriod.Contains("D")) {
   dataNames.push_back("Single El RunD");
   datafiles.push_back(TFile::Open((leg3Dir+"SingleEl_RunD.root").c_str(), "READ"));
   weight_data.push_back( 1 ); //
   }
   //      }
   }
  ...
  
  {
        ...
        for (UInt_t c=0; ((UInt_t)(1<<c))<=chan+1; c++) {
          ...
          if (((channel+1)&(chan+1))!=0) {
            ...
            for (UInt_t k=0; k<files[0].size(); k++) {
              TEfficiency *eff_tmp = (TEfficiency*) fBtagEff->Get((samples_labelBQ[0][k]+flavourKind[fl]+channelLabel[c]).c_str());
              printf("%s ",(samples_labelBQ[0][k]+flavourKind[fl]+channelLabel[c]).c_str());
             ...
             */
            Double_t x=0. , ynom=0. , yminus=0., yplus=0. ;
            Double_t ylow=0. , yhig=0. ;
            if (tgn != NULL) {
      ...
      for (UInt_t fl=0; fl<3; fl++) {
        ...
        for (UInt_t c=0; ((UInt_t)(1<<c))<=chan+1; c++) {
          ...
          for (UInt_t k=0; k<files[1].size(); k++) {
            TEfficiency *eff_tmp = (TEfficiency*) fBtagEff->Get((samples_labelBQ[1][k]+flavourKind[fl]+channelLabel[c]).c_str());
            printf("%s ",(samples_labelBQ[1][k]+flavourKind[fl]+channelLabel[c]).c_str());
            TEfficiency *eff1bin = NULL;
            if (eff_tmp!=NULL) {
              eff1bin = average(eff_tmp, (fl==2?5:(fl==1?4:0)), std::string(runPeriod.Data()), 0, kFALSE);
              if (nominalListWasEmpty == kTRUE) {
        ...
        if (fl==0 || fl==1 || fl==2) { // To make the guds and c categories splitted
          printf("\n");
          weights.clear();
          effListNominal->Clear();
          effListPlus->Clear();
    ...
    for(UInt_t chan=0; chan<NbOfChannels; chan++) {
      for (UInt_t c=0; ((UInt_t)(1<<c))<=chan+1; c++) {
        ...
        for (UInt_t i=0; i<NbOfJetBins; i++) {
          for (UInt_t j=0; j<NbOfWP; j++) {
            printf("%s\n", (std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str());
              //ttlike
            printf(" TTlike");
            if (files[0].size() != 0) {
              if (hNbtaggedJets_ttlike[chan][i][j]==NULL) {
                hNbtaggedJets_ttlike[chan][i][j] = (TH1D*) files[0][0]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())/*->Clone()*/;
                hNbtaggedJets_ttlike[chan][i][j]->SetDirectory(NULL);
                hNbtaggedJets_ttlike[chan][i][j]->Reset();
              }
              for (UInt_t k=0; k<files[0].size(); k++) {
                hNbtaggedJets_ttlike[chan][i][j]->Add((TH1D*) files[0][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), IntLumi[c]*samples_weights[0][k]);
              }
              if (j==0) {
                printf("(BQuarks)");
                /*
                ...
                */
                 if (eXbq__[chan][i]==NULL) {
                 eXbq__[chan][i] = (TH1D*) files[0][0]->Get((std::string()+"B_Quarks"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())->Clone();
                 ...
                 for (UInt_t k=0; k<files[0].size(); k++) {
                 printf("%s\n",((TH1D*) files[0][k]->Get((std::string()+"B_Quarks"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()))->GetName());
                 eXbq__[chan][i]->Add( ((TH1D*) files[0][k]->Get((std::string()+"B_Quarks"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())), IntLumi[c]*samples_weights[0][k]); //eXbq
            ...
            if (files[1].size() != 0) {
              if (hNbtaggedJets_vlike[chan][i][j]==NULL) {
                hNbtaggedJets_vlike[chan][i][j] = (TH1D*) files[1][0]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())/*->Clone()*/;
                hNbtaggedJets_vlike[chan][i][j]->SetDirectory(NULL);
                hNbtaggedJets_vlike[chan][i][j]->Reset();
              }
              for (UInt_t k=0; k<files[1].size(); k++) {
                hNbtaggedJets_vlike[chan][i][j]->Add((TH1D*) files[1][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), IntLumi[c]*samples_weights[1][k]);
                  //                (TH1D*) files[1][0]->Get((std::string()+"B_Jet_Multiplicity"+wpSuffix[j]).c_str());
              }
              if (i==0) {
                printf("(BTaggingEff)");
            ...
            if (files[2].size() != 0) {
              if (hNbtaggedJets_vbjets[chan][i][j]==NULL) {
                hNbtaggedJets_vbjets[chan][i][j] = (TH1D*) files[2][0]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())/*->Clone()*/;
                hNbtaggedJets_vbjets[chan][i][j]->SetDirectory(NULL);
                hNbtaggedJets_vbjets[chan][i][j]->Reset();
              }
              for (UInt_t k=0; k<files[2].size(); k++) {
                hNbtaggedJets_vbjets[chan][i][j]->Add((TH1D*) files[2][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), IntLumi[c]*samples_weights[2][k]);
                hNbtaggedJets_vlike[chan][i][j]->Add((TH1D*) files[2][k]->Get((std::string()+"B_Jet_Multiplicity_bb"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), -1. * IntLumi[c]*samples_weights[2][k]);
              }
            }
            ...
            if (files[3].size() != 0) {
              if (hNbtaggedJets_stjets[chan][i][j]==NULL) {
                hNbtaggedJets_stjets[chan][i][j] = (TH1D*) files[3][0]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())/*->Clone()*/;
                hNbtaggedJets_stjets[chan][i][j]->SetDirectory(NULL);
                hNbtaggedJets_stjets[chan][i][j]->Reset();
              }
              for (UInt_t k=0; k<files[3].size(); k++) {
                hNbtaggedJets_stjets[chan][i][j]->Add((TH1D*) files[3][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), IntLumi[c]*samples_weights[3][k]);
              }
            }
            ...
            if (files[4].size() != 0) {
              if (hNbtaggedJets_vvjets[chan][i][j]==NULL) {
                hNbtaggedJets_vvjets[chan][i][j] = (TH1D*) files[4][0]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())/*->Clone()*/;
                hNbtaggedJets_vvjets[chan][i][j]->SetDirectory(NULL);
                hNbtaggedJets_vvjets[chan][i][j]->Reset();
              }
              
              for (UInt_t k=0; k<files[4].size(); k++) {
                hNbtaggedJets_vvjets[chan][i][j]->Add((TH1D*) files[4][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), IntLumi[c]*samples_weights[4][k]);
              }
            }
            
            printf(" MCPerProcess");
            for (UInt_t f=0; f<5; f++) {
              for (UInt_t k=0; k<files[f].size(); k++) {
                if(samples_names[f][k].find("TTJets")==0) {
                  if (hNbtaggedJets_ttjets[chan][i][j]==NULL) {
                    hNbtaggedJets_ttjets[chan][i][j] = (TH1D*) files[f][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())/*->Clone()*/;
                    hNbtaggedJets_ttjets[chan][i][j]->SetDirectory(NULL);
                    hNbtaggedJets_ttjets[chan][i][j]->Reset();
                  }
                  hNbtaggedJets_ttjets[chan][i][j]->Add((TH1D*) files[f][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), IntLumi[c]*samples_weights[f][k]);
                }
                if(samples_names[f][k].find("WJets")==0) {
                  if (hNbtaggedJets_wjets[chan][i][j]==NULL) {
                    hNbtaggedJets_wjets[chan][i][j] = (TH1D*) files[f][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())/*->Clone()*/;
                    hNbtaggedJets_wjets[chan][i][j]->SetDirectory(NULL);
                    hNbtaggedJets_wjets[chan][i][j]->Reset();
                  }
                  hNbtaggedJets_wjets[chan][i][j]->Add((TH1D*) files[f][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), IntLumi[c]*samples_weights[f][k]);
                }
                if(samples_names[f][k].find("DYJets")==0) {
                  if (hNbtaggedJets_zjets[chan][i][j]==NULL) {
                    hNbtaggedJets_zjets[chan][i][j] = (TH1D*) files[f][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())/*->Clone()*/;
                    hNbtaggedJets_zjets[chan][i][j]->SetDirectory(NULL);
                    hNbtaggedJets_zjets[chan][i][j]->Reset();
                  }
                  hNbtaggedJets_zjets[chan][i][j]->Add((TH1D*) files[f][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), IntLumi[c]*samples_weights[f][k]);
                }
                if(samples_names[f][k].find("QCD")==0) {
                  ...
                  if (hNbtaggedJets_qcd[chan][i][j]==NULL) {
                    hNbtaggedJets_qcd[chan][i][j] = (TH1D*) files[f][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str())/*->Clone()*/;
                    hNbtaggedJets_qcd[chan][i][j]->SetDirectory(NULL);
                    hNbtaggedJets_qcd[chan][i][j]->Reset();
                  }
                  hNbtaggedJets_qcd[chan][i][j]->Add((TH1D*) files[f][k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), IntLumi[c]*samples_weights[f][k]);
                }
            ...
            if (datafiles.size() != 0) {
              if (isOnData) {
                ...
                if (hNbtaggedJets[chan][i][j]==NULL) {
                  hNbtaggedJets[chan][i][j] = (TH1D*) datafiles[0]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str());
                ...
                for (UInt_t k=0; k<datafiles.size(); k++) {
                  ...
                  hNbtaggedJets[chan][i][j]->Add((TH1D*) datafiles[k]->Get((std::string()+"B_Jet_Multiplicity"+channelLeg3Suffix[c]+jetSuffix[i]+wpSuffix[j]).c_str()), weight_data[k]);
  ...
  
  {
    ...
    for (UInt_t c=0; c<NbOfChannels; c++) {
      ...
      for (UInt_t j=0; j<NbOfWP; j++) {
        ...
        Euds_const_mean[c][j] = effV[0][c];
        Euds_const_error[c][j] = effV[0][c];
        Eudsc_const_mean[c][j] = effTT[1][c];
        Eudsc_const_error[c][j] = effTT[1][c];
...
/*
 ...
 g++ -O -g $(root-config --cflags) $(root-config --libs)  -lMathMore -lFoam -lMinuit -lRooFitCore -lRooFit -o StopSearches_VJetsBckgdEst.exe StopSearches_VJetsBckgdEst_8TeV.cc
 */
