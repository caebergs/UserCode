...
cp ./wholeChain.sh ${timestamp}_workingDir/wholeChain.sh ;
cd ${timestamp}_workingDir ;
echo "cd ${timestamp}_workingDir"
echo -e "\n --> Extracting content from the V+jets estimation ....\n"

cp ../default_input_values.txt ./default_input_values.txt ;
...
EOD
    ...
    for uncert in -1 0 1 2 3 ; do
      for fluctDirect in 0 1 ; do
        ...
        iCont=$(cat default_input_values.txt | grep "RV_syst_uncert_rel\[${useCase}\]\[$((${uncert}+1))\]\[${fluctDirect}\]\[$((${channel}-1))\]=" | sed -e "s/^[ ]*RV_syst_uncert_rel\[${useCase}\]\[$((${uncert}+1))\]\[${fluctDirect}\]\[$((${channel}-1))\]=[ ]*\([0-9\.+\-]*\)[ ]*;.*/\1/") ;
        echo "iCont [${useCase}][$((${uncert}+1))][${fluctDirect}][${channel}] = ${iCont}"
        declare -i iBin ;
        iBin=${uncert}*2+${fluctDirect} ;
        if [ "${uncert}" = "323" ] ; then
