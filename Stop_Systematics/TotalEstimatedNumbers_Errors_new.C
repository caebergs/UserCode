...
double CorrCoef( double N, double sigmaN, double R, double sigmaR);
double ErrorOnProd( double N, double sigmaN, double R, double sigmaR, double Nest, double sigmaNest);

void TotalEstimatedNumbers_Errors_new(string filename, int UseCase, int bin,bool UseWNJets){
  printf("a\n");
  ...
  // W+jets
  BckgdNames[1] = "w+jets";
  inputfiles[1][0] = TFile::Open((path+"Wjets_mu.root").c_str());
  inputfiles[1][1] = TFile::Open((path+"Wjets_el.root").c_str());

  // Z+jets
  BckgdNames[2] = "z+jets";
  inputfiles[2][0] = TFile::Open((path+"Zjets_mu.root").c_str());
  inputfiles[2][1] = TFile::Open((path+"Zjets_el.root").c_str());

  // st+jets
  BckgdNames[3] = "st+jets (s-ch)";
  inputfiles[3][0] = TFile::Open((path+"TopS_mu.root").c_str());
  inputfiles[3][1] = TFile::Open((path+"TopS_el.root").c_str());
  
  BckgdNames[4] = "stbar+jets (s-ch)";
  inputfiles[4][0] = TFile::Open((path+"TopbarS_mu.root").c_str());
  inputfiles[4][1] = TFile::Open((path+"TopbarS_el.root").c_str());
  
  BckgdNames[5] = "st+jets (t-ch)";
  inputfiles[5][0] = TFile::Open((path+"TopT_mu.root").c_str());
  inputfiles[5][1] = TFile::Open((path+"TopT_el.root").c_str());
  
  BckgdNames[6] = "stbar+jets (t-ch)";
  inputfiles[6][0] = TFile::Open((path+"TopbarT_mu.root").c_str());
  inputfiles[6][1] = TFile::Open((path+"TopbarT_el.root").c_str());
  
  BckgdNames[7] = "st+jets (tW-ch)";
  inputfiles[7][0] = TFile::Open((path+"TopTW_mu.root").c_str());
  inputfiles[7][1] = TFile::Open((path+"TopTW_el.root").c_str());
  
  BckgdNames[8] = "stbar+jets (tW-ch)";
  inputfiles[8][0] = TFile::Open((path+"TopTW_mu.root").c_str());
  inputfiles[8][1] = TFile::Open((path+"TopTW_el.root").c_str());

  // di-boson
  BckgdNames[9] = "WW+jets";
  inputfiles[9][0] = TFile::Open((path+"WW_mu.root").c_str());
  inputfiles[9][1] = TFile::Open((path+"WW_el.root").c_str());
  BckgdNames[10] = "WZ+jets";
  inputfiles[10][0]= TFile::Open((path+"WZ_mu.root").c_str());
  inputfiles[10][1]= TFile::Open((path+"WZ_el.root").c_str());
  BckgdNames[11] = "ZZ+jets";
  inputfiles[11][0]= TFile::Open((path+"ZZ_mu.root").c_str());
  inputfiles[11][1]= TFile::Open((path+"ZZ_el.root").c_str());

  // W+jets exclusive
  BckgdNames[12] = "W2+jets";
  inputfiles[12][0]= TFile::Open((path+"W2Jets_mu.root").c_str());
  inputfiles[12][1]= TFile::Open((path+"W2Jets_el.root").c_str());
  BckgdNames[13] = "W3+jets";
  inputfiles[13][0]= TFile::Open((path+"W3Jets_mu.root").c_str());
  inputfiles[13][1]= TFile::Open((path+"W3Jets_el.root").c_str());
  BckgdNames[14] = "W4+jets";
  inputfiles[14][0]= TFile::Open((path+"W4Jets_mu.root").c_str());
  inputfiles[14][1]= TFile::Open((path+"W4Jets_el.root").c_str());
  ...
  for(int i=0;i<NbOfFiles;i++){
    tEff[i][0] = (TEfficiency*)inputfiles[i][0]->Get(effname.c_str());
    tEff[i][1] = (TEfficiency*)inputfiles[i][1]->Get(effname.c_str());
    tEff[i][0]->SetStatisticOption(TEfficiency::kBBayesian);
    tEff[i][1]->SetStatisticOption(TEfficiency::kBBayesian);

    pList->Add((TEfficiency*)tEff[i][0]);
    pList->Add((TEfficiency*)tEff[i][1]);

    tg = (TGraphAsymmErrors*)tDummy->Combine(pList);//,"",2);
//    tg->GetPoint(bin[UseCase],x,y[i]);
//    yerr_low[i]  = tg->GetErrorYlow(bin[UseCase]);
//    yerr_high[i] = tg->GetErrorYhigh(bin[UseCase]);
    tg->GetPoint(bin,x,y[i]);
  ...
  // tt+jets
  TGraphAsymmErrors *tg_ttjets;
  double *w_ttjets = 0;
  // muon channel
  pList->Add(((TEfficiency*)tEff[0][0]));
  // electron channel
  pList->Add(((TEfficiency*)tEff[0][1]));
  // weights
  w_ttjets = new double[2];
  w_ttjets[0] = 26500/tEff[0][0]->GetTotalHistogram()->GetEntries();//165;
  w_ttjets[1] = 19666/tEff[0][1]->GetTotalHistogram()->GetEntries();//165;
  tg_ttjets = (TGraphAsymmErrors*)tDummy->Combine(pList,"mode",2);//,w_ttjets);

//  tg_vjets->GetPoint(bin[UseCase],x,y_vjets);
//  yerr_low_vjets  = tg_vjets->GetErrorYlow(bin[UseCase]);
//  yerr_high_vjets = tg_vjets->GetErrorYhigh(bin[UseCase]);
  tg_ttjets->GetPoint(bin,x,y_ttlike);
  yerr_low_ttlike  = tg_ttjets->GetErrorYlow(bin);
  yerr_high_ttlike = tg_ttjets->GetErrorYhigh(bin);
/*  
  y_ttlike                   = y[0];         //tEff[0]->GetEfficiency(bin[UseCase]);
  yerr_low_ttlike            = yerr_low[0];  //tEff[0]->GetEfficiencyErrorLow(bin[UseCase]);
  yerr_high_ttlike           = yerr_high[0]; //tEff[0]->GetEfficiencyErrorUp(bin[UseCase]);
*/
  pList->Clear();
  ...
  if(!UseWNJets){
    // muon channel
    pList->Add(((TEfficiency*)tEff[1][0])); // W+jets
    pList->Add((TEfficiency*)tEff[2][0]);   // Z+jets
    // electron channel
    pList->Add(((TEfficiency*)tEff[1][1])); // W+jets
    pList->Add((TEfficiency*)tEff[2][1]);   // Z+jets
    // weights
    w_vjets = new double[4];
    w_vjets[0] = 18934/tEff[1][0]->GetTotalHistogram()->GetEntries();//31314;
    w_vjets[1] = 1400/tEff[2][0]->GetTotalHistogram()->GetEntries();//3048;
    w_vjets[2] = 14970/tEff[1][1]->GetTotalHistogram()->GetEntries();//31314;
    w_vjets[3] = 2223/tEff[2][1]->GetTotalHistogram()->GetEntries();//3048;
    //double w_vjets[4] = {31314,3048,31314,3048};
  }
  else{
    // muon channel
    pList->Add(((TEfficiency*)tEff[12][0])); // W2+jets
    pList->Add(((TEfficiency*)tEff[13][0])); // W3+jets
    pList->Add(((TEfficiency*)tEff[14][0])); // W4+jets
    pList->Add((TEfficiency*)tEff[2][0]);    // Z+jets
    // electron channel
    pList->Add(((TEfficiency*)tEff[12][1])); // W2+jets
    pList->Add(((TEfficiency*)tEff[13][1])); // W3+jets
    pList->Add(((TEfficiency*)tEff[14][1])); // W4+jets
    pList->Add((TEfficiency*)tEff[2][1]);    // Z+jets
    // weights
    w_vjets = new double[8];
    w_vjets[0] =   168.1/tEff[12][0]->GetTotalHistogram()->GetEntries();/*1618.1;*/  w_vjets[4] =   124.5/tEff[12][1]->GetTotalHistogram()->GetEntries();//1618.1;
    w_vjets[1] =   327.1/tEff[13][0]->GetTotalHistogram()->GetEntries();/*343.2;*/   w_vjets[5] =   233.5/tEff[13][1]->GetTotalHistogram()->GetEntries();//343.2;
    w_vjets[2] = 16870.0/tEff[14][0]->GetTotalHistogram()->GetEntries();/*194.6;*/   w_vjets[6] = 13643.0/tEff[14][1]->GetTotalHistogram()->GetEntries();//194.6;
    w_vjets[3] =  1400.0/tEff[2][0]->GetTotalHistogram()->GetEntries();              w_vjets[7] =  2223.0/tEff[2][1]->GetTotalHistogram()->GetEntries();
  }
  tg_vjets = (TGraphAsymmErrors*)tDummy->Combine(pList,"mode",(UseWNJets ? 8:4),w_vjets);
//  tg_vjets->GetPoint(bin[UseCase],x,y_vjets);
//  yerr_low_vjets  = tg_vjets->GetErrorYlow(bin[UseCase]);
//  yerr_high_vjets = tg_vjets->GetErrorYhigh(bin[UseCase]);
  tg_vjets->GetPoint(bin,x,y_vjets);
  yerr_low_vjets  = tg_vjets->GetErrorYlow(bin);
  yerr_high_vjets = tg_vjets->GetErrorYhigh(bin);
  pList->Clear();
  /*******************************************************************************************************************************/
  /*******************************************************************************************************************************/
/*
  double tmpval[4] = {0.001789,0.001605,0.007944,0.010535};
  double tmperr_high[4] = {0.000212,0.000201,0.000446,0.000513};
  double tmperr_low[4] = {0.000212,0.000201,0.000446,0.000513};
  y_vjets = tmpval[UseCase];
  yerr_high_vjets = tmperr_high[UseCase];
  yerr_low_vjets  = tmperr_low[UseCase];
*/
  /*******************************************************************************************************************************/
  /*******************************************************************************************************************************/
...
//  double RV_syst_err_low[4] = {0.000171,0.000140,0.000054,0.000000}; // Down
  ...
  // muon channel
  pList->Add((TEfficiency*)tEff[3][0]); // st+jets s-ch
  pList->Add((TEfficiency*)tEff[4][0]); // st+jets s-ch
  pList->Add((TEfficiency*)tEff[5][0]); // st+jets t-ch
  pList->Add((TEfficiency*)tEff[6][0]); // st+jets t-ch
  pList->Add((TEfficiency*)tEff[7][0]); // st+jets tW-ch
  pList->Add((TEfficiency*)tEff[8][0]); // st+jets tW-ch
  // electron channel
  pList->Add((TEfficiency*)tEff[3][1]); // st+jets s-ch
  pList->Add((TEfficiency*)tEff[4][1]); // st+jets s-ch
  pList->Add((TEfficiency*)tEff[5][1]); // st+jets t-ch
  pList->Add((TEfficiency*)tEff[6][1]); // st+jets t-ch
  pList->Add((TEfficiency*)tEff[7][1]); // st+jets tW-ch
  pList->Add((TEfficiency*)tEff[8][1]); // st+jets tW-ch
  // weights
  double w_stjets[12];
  w_stjets[ 0] =  27.02/tEff[3][0]->GetTotalHistogram()->GetEntries();
  w_stjets[ 1] =  12.40/tEff[4][0]->GetTotalHistogram()->GetEntries();
  w_stjets[ 2] = 243.65/tEff[5][0]->GetTotalHistogram()->GetEntries();
  w_stjets[ 3] = 133.98/tEff[6][0]->GetTotalHistogram()->GetEntries();
  w_stjets[ 4] =1115.05/tEff[7][0]->GetTotalHistogram()->GetEntries();
  w_stjets[ 5] =1115.05/tEff[8][0]->GetTotalHistogram()->GetEntries();
  w_stjets[ 6] =  20.33/tEff[3][1]->GetTotalHistogram()->GetEntries();
  w_stjets[ 7] =   8.83/tEff[4][1]->GetTotalHistogram()->GetEntries();
  w_stjets[ 8] = 175.97/tEff[5][1]->GetTotalHistogram()->GetEntries();
  w_stjets[ 9] =  94.45/tEff[6][1]->GetTotalHistogram()->GetEntries();
  w_stjets[10] = 870.01/tEff[7][1]->GetTotalHistogram()->GetEntries();
  w_stjets[11] = 870.01/tEff[8][1]->GetTotalHistogram()->GetEntries();
  tg_stjets = (TGraphAsymmErrors*)tDummy->Combine(pList,"mode",12,w_stjets);
...
//  yerr_high_stjets = tg_stjets->GetErrorYhigh(bin[UseCase]);
  tg_stjets->GetPoint(bin,x,y_stjets);
  yerr_low_stjets  = tg_stjets->GetErrorYlow(bin);
  yerr_high_stjets = tg_stjets->GetErrorYhigh(bin);
  pList->Clear();
  ...
  // muon channel
  pList->Add((TEfficiency*)tEff[9][0]);  // WW+jets
  pList->Add((TEfficiency*)tEff[10][0]); // WZ+jets
  pList->Add((TEfficiency*)tEff[11][0]); // ZZ+jets
  // electron channel
  pList->Add((TEfficiency*)tEff[9][1]);  // WW+jets
  pList->Add((TEfficiency*)tEff[10][1]); // WZ+jets
  pList->Add((TEfficiency*)tEff[11][1]); // ZZ+jets
  // weights
  double w_vvjets[6];
  w_vvjets[0] = 225.10/tEff[9][0]->GetTotalHistogram()->GetEntries();
  w_vvjets[1] =  68.48/tEff[10][0]->GetTotalHistogram()->GetEntries();
  w_vvjets[2] =   7.12/tEff[11][0]->GetTotalHistogram()->GetEntries();
  w_vvjets[3] = 175.97/tEff[9][1]->GetTotalHistogram()->GetEntries();
  w_vvjets[4] =  67.74/tEff[10][1]->GetTotalHistogram()->GetEntries();
  w_vvjets[5] =  11.31/tEff[11][1]->GetTotalHistogram()->GetEntries();

  tg_vvjets = (TGraphAsymmErrors*)tDummy->Combine(pList,"mode",6,w_vvjets);
...
//  yerr_high_vvjets = tg_vvjets->GetErrorYhigh(bin[UseCase]);
  tg_vvjets->GetPoint(bin,x,y_vvjets);
  yerr_low_vvjets  = tg_vvjets->GetErrorYlow(bin);
  yerr_high_vvjets = tg_vvjets->GetErrorYhigh(bin);
  pList->Clear();
  cout<<"R_{VV} = ("<<y_vvjets*1000<<"^{+"<<yerr_high_vvjets*1000<<"}_{-"<<yerr_low_vvjets*1000<<"})\\cdot 10^{-3}"<<endl;

  // Multi-jets
  TGraphAsymmErrors *tg_multijets;
  double qcd[6] = {1281.98,0.0,2205.11,517.63,131.28,36.42};
  //{84679.3,3625840.,142813.8,3263.436,368.01,55.016};
  vector<double> weights_multijet;

  for(int i=0;i<6;i++){
    tDummy = (TEfficiency*)inputfiles_multijets[i]->Get(effname.c_str());
    if(tDummy->GetEfficiency(bin+1)>0){
      cout<<"Multi-jet "<<i<<" : eff = "<<tDummy->GetEfficiency(bin+1)<<endl;
      pList->Add(tDummy);
      weights_multijet.push_back(qcd[i]/tDummy->GetTotalHistogram()->GetEntries());
    }
  }
  //double *w_qcd = new double[weights_multijet.size()];
  //for(unsigned int ii=0;ii<weights_multijet.size();ii++) w_qcd[ii] = weights_multijet[i];
  if(weights_multijet.size()>0){
    tg_multijets = (TGraphAsymmErrors*)tDummy->Combine(pList,"mode",weights_multijet.size(),&weights_multijet[0]);
//  tg_multijets->GetPoint(bin[UseCase],x,y_multijets);
//  yerr_low_multijets  = tg_multijets->GetErrorYlow(bin[UseCase]);
//  yerr_high_multijets = tg_multijets->GetErrorYhigh(bin[UseCase]);
    tg_multijets->GetPoint(bin,x,y_multijets);
    yerr_low_multijets  = tg_multijets->GetErrorYlow(bin);
    yerr_high_multijets = tg_multijets->GetErrorYhigh(bin);
  ...
  Coef_Up[0] = CorrCoef(y_stjets,yerr_high_stjets,nbOfEvents[0],statUncert[0]);
  Coef_Up[1] = CorrCoef(y_stjets,yerr_high_stjets,nbOfEvents[1],statUncert[1]);
  Coef_Up[2] = CorrCoef(y_stjets,yerr_high_stjets,nbOfEvents[2],statUncert[2]);
  Coef_Up[3] = CorrCoef(y_ttlike,yerr_high_ttlike,nbOfEvents[3],sqrt(pow(statUncert[3],2)+pow(systUncert[3],2)));
  Coef_Up[4] = CorrCoef(y_ttlike,yerr_high_ttlike,nbOfEvents[4],sqrt(pow(statUncert[4],2)+pow(systUncert[4],2)));
  Coef_Up[5] = CorrCoef(y_ttlike,yerr_high_ttlike,nbOfEvents[5],sqrt(pow(statUncert[5],2)+pow(systUncert[5],2)));
  Coef_Up[6] = CorrCoef(y_vjets, yerr_high_vjets, nbOfEvents[6]+nbOfEvents[9]-QCDEst*0.7,sqrt(pow(statUncert[6],2)+pow(systUncert[6],2)+pow(statUncert[ 9],2)+pow(systUncert[ 9],2)));
  Coef_Up[7] = CorrCoef(y_vjets, yerr_high_vjets, nbOfEvents[7]+nbOfEvents[10]-QCDEst*0.25,sqrt(pow(statUncert[7],2)+pow(systUncert[7],2)+pow(statUncert[10],2)+pow(systUncert[10],2)));
  Coef_Up[8] = CorrCoef(y_vjets, yerr_high_vjets, nbOfEvents[8]+nbOfEvents[11]-QCDEst*0.05,sqrt(pow(statUncert[8],2)+pow(systUncert[8],2)+pow(statUncert[11],2)+pow(systUncert[11],2)));
  Coef_Up[9] = 0;
  Coef_Up[10]= 0;
  Coef_Up[11]= 0;
  Coef_Up[12]= CorrCoef(y_vvjets, yerr_high_vvjets, nbOfEvents[12],statUncert[12]);
  Coef_Up[13]= CorrCoef(y_vvjets, yerr_high_vvjets, nbOfEvents[13],statUncert[13]);
  Coef_Up[14]= CorrCoef(y_vvjets, yerr_high_vvjets, nbOfEvents[14],statUncert[14]);
  
  Coef_Down[0] = CorrCoef(y_stjets,yerr_low_stjets,nbOfEvents[0],statUncert[0]);
  Coef_Down[1] = CorrCoef(y_stjets,yerr_low_stjets,nbOfEvents[1],statUncert[1]);
  Coef_Down[2] = CorrCoef(y_stjets,yerr_low_stjets,nbOfEvents[2],statUncert[2]);
  Coef_Down[3] = CorrCoef(y_ttlike,yerr_low_ttlike,nbOfEvents[3],sqrt(pow(statUncert[3],2)+pow(systUncert[3],2)));
  Coef_Down[4] = CorrCoef(y_ttlike,yerr_low_ttlike,nbOfEvents[4],sqrt(pow(statUncert[4],2)+pow(systUncert[4],2)));
  Coef_Down[5] = CorrCoef(y_ttlike,yerr_low_ttlike,nbOfEvents[5],sqrt(pow(statUncert[5],2)+pow(systUncert[5],2)));
  Coef_Down[6] = CorrCoef(y_vjets, yerr_low_vjets, nbOfEvents[6]+nbOfEvents[9]-QCDEst*0.7,sqrt(pow(statUncert[6],2)+pow(systUncert[6],2)+pow(statUncert[ 9],2)+pow(systUncert[ 9],2)));
  Coef_Down[7] = CorrCoef(y_vjets, yerr_low_vjets, nbOfEvents[7]+nbOfEvents[10]-QCDEst*0.25,sqrt(pow(statUncert[7],2)+pow(systUncert[7],2)+pow(statUncert[10],2)+pow(systUncert[10],2)));
  Coef_Down[8] = CorrCoef(y_vjets, yerr_low_vjets, nbOfEvents[8]+nbOfEvents[11]-QCDEst*0.05,sqrt(pow(statUncert[8],2)+pow(systUncert[8],2)+pow(statUncert[11],2)+pow(systUncert[11],2)));
  Coef_Down[9] = 0;
  Coef_Down[10]= 0;
  Coef_Down[11]= 0;
  Coef_Down[12]= CorrCoef(y_vvjets, yerr_low_vvjets, nbOfEvents[12],statUncert[12]);
  Coef_Down[13]= CorrCoef(y_vvjets, yerr_low_vvjets, nbOfEvents[13],statUncert[13]);
  Coef_Down[14]= CorrCoef(y_vvjets, yerr_low_vvjets, nbOfEvents[14],statUncert[14]);
...
/*
  for(i=0;i<15;i++){
    ...
    if(i>2 && i<6) ErrNtt_sel[3] += pow(TotUncert_Up[i],2);
    for(int j=0;j<15;j++){
      if(j==i) continue;
      //cout<<"corr("<<i<<","<<j<<") = "<<corrMatrix[i][j]<<endl;
      //cout<<" new corr("<<i<<","<<j<<") = "<<corrMatrix[i][j]*Coef_Up[i]*Coef_Up[j]<<endl;
      //Ntot_err_up += corrMatrix[i][j]*TotUncert_Up[i]*TotUncert_Up[j];
      //Ntot_err_low+= corrMatrix[i][j]*TotUncert_Low[i]*TotUncert_Low[j];
      Ntot_err_up += corrMatrix[i][j]*Coef_Up[i]*Coef_Up[j]*TotUncert_Up[i]*TotUncert_Up[j];
      Ntot_err_low+= corrMatrix[i][j]*Coef_Down[i]*Coef_Down[j]*TotUncert_Low[i]*TotUncert_Low[j];
      if(i>2 && i<6 && j>2 && j<6) ErrNtt_sel[3] += corrMatrix[i][j]*Coef_Up[i]*Coef_Up[j]*TotUncert_Up[i]*TotUncert_Up[j];
    }
  }
...
*/
  ...
  cout<<"Averaged Correlation matrix 5X5 :"<<endl;
  cout<<"Corr     | ST+jets | TT+jets | V+jets | Vbb | VV+jets "<<endl;
  cout<<" ST+jets |"<<MeanCorr[0][0]<<" | "<<MeanCorr[0][1]<<" | "<<MeanCorr[0][2]<<" | "<<MeanCorr[0][3]<<" | "<<MeanCorr[0][4]<<endl;
  cout<<" TT+jets |"<<MeanCorr[1][0]<<" | "<<MeanCorr[1][1]<<" | "<<MeanCorr[1][2]<<" | "<<MeanCorr[1][3]<<" | "<<MeanCorr[1][4]<<endl;
  cout<<"  V+jets |"<<MeanCorr[2][0]<<" | "<<MeanCorr[2][1]<<" | "<<MeanCorr[2][2]<<" | "<<MeanCorr[2][3]<<" | "<<MeanCorr[2][4]<<endl;
  cout<<" Vbb     |"<<MeanCorr[3][0]<<" | "<<MeanCorr[3][1]<<" | "<<MeanCorr[3][2]<<" | "<<MeanCorr[3][3]<<" | "<<MeanCorr[3][4]<<endl;
  cout<<" VV+jets |"<<MeanCorr[4][0]<<" | "<<MeanCorr[4][1]<<" | "<<MeanCorr[4][2]<<" | "<<MeanCorr[4][3]<<" | "<<MeanCorr[4][4]<<endl;
...
//  cout<<"Nttlike*RTT = "<<Nttlike*y_ttlike<<"+/-"<<sqrt(pow(TotUncert_Up[3],2)+pow(TotUncert_Up[4],2)+pow(TotUncert_Up[5],2))<<endl;
/*
  cout<<"Nttlike*RTT = "<<Ntt_sel[3]<<"^{+"<<ErrNtt_sel[3]<<"}"<<endl;
  cout<<"(Nvlike-Nqcd+Nvbjets)*RV = "<<(Nvlike-QCDEst+Nvbjets)*y_vjets<<"+/-"<<sqrt(pow(TotUncert_Up[6],2)+pow(TotUncert_Up[7],2)+pow(TotUncert_Up[8],2))<<endl;
...
*/
  ...
  for(int i=0;i<NbOfFiles;i++){
    inputfiles[i][0]->Close();
    inputfiles[i][1]->Close();
  }
...
double ErrorOnProd(double A, double sigmaA, double B, double sigmaB){
  if(A<=0 || B<=0) return 0;
  double sigmaC = (A*B)*sqrt(pow(sigmaA/A,2)+pow(sigmaB/B,2));
  return sigmaC;
}
